                    ...
                    <div class="search-box w-100">
                        <form>
                            <input type="search" class="form-control form--control w-100 h-36" name="search"
                                ...
                                <span class="search__icon"><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M13.9998 14L11.1064 11.1067" stroke="#99A1AF" stroke-width="1.33333" stroke-linecap="round" stroke-linejoin="round"/>
    ...
    <script>
        (function ($) {
            ...
            $('.view-btn').on('click', function () {
                ...
                var isPending = user.company_complete == 1 && user.profile_complete == 1 && user
                    .id_verification_complete == 1 && user.is_verified == 0;
                var isVerified = user.is_verified == 1;
                var isIncomplete = !isPending && !isVerified;

                if (isVerified) {
                    statusBadge.text('Verified').removeClass('badge--warning badge--danger').addClass(
                        'badge--success');
                    modal.find('.pending-user-actions').addClass('d-none');
                    modal.find('.verified-user-actions').removeClass('d-none');
                } else if (isPending) {
                    statusBadge.text('Pending').removeClass('badge--success badge--danger').addClass(
                        'badge--warning');
                    modal.find('.pending-user-actions').removeClass('d-none');
                    modal.find('.verified-user-actions').addClass('d-none');
                } else if (isIncomplete) {
                    statusBadge.text('Incomplete').removeClass('badge--success badge--warning').addClass(
                        'badge--info');
                    modal.find('.pending-user-actions').addClass('d-none');
                    modal.find('.verified-user-actions').addClass('d-none');
                }
