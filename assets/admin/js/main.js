...
(function ($) {
  ...
  $(".image-uploader").on("change", ".image-upload-input", function (e) {
    ...
    const file = files[0];
    const supportedFiles = $this.attr("accept")
      ? $this.attr("accept").split(",")
      : [];
    const formattedSupportedFiles = supportedFiles.map(
      (supportedFile) =>
        `image/${supportedFile.replace(".", "").replace(" ", "")}`
    );
  ...
  $(".highLightSearchInput").on("input", function (e) {
    ...
    $.each($(hightLightParentSelector), function (i, element) {
      ...
      if (sectionNameToLower.includes(searchValue)) {
        ...
        const highLightText = sectionName.replace(
          regex,
...
const getAmount = (amount, precision = null) => {
  ...
  if (floAmount % 1 === 0) return floAmount.toString();
  const allowPrecision = precision
    ? precision
    : window.app_config.allow_precision;
  return floAmount.toFixed(allowPrecision);
};
